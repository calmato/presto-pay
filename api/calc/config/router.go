...
func Router(reg *registry.Registry) *gin.Engine {
	...
	apiV1 := r.Group("/v1")
	{
		...
		apiV1.PATCH("/groups/:groupID/hidden", reg.V1Group.AddHiddenGroup)
		apiV1.DELETE("/groups/:groupID/hidden", reg.V1Group.RemoveHiddenGroup)
		...
		groups := apiV1.Group("/groups/:groupID")
		{
			...
			groups.PATCH("/payments-status", reg.V1Payment.UpdateStatusAll)
			groups.PATCH("/payments-status/:paymentID", reg.V1Payment.UpdateStatus)
